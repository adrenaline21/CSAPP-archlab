...
#ifndef HAS_GUI
        printf(
            "To run in GUI mode, you must recompile with the HAS_GUI constant "
            "defined.\n");
        exit(1);
#endif /* HAS_GUI */
...
static void run_tty_sim() {
    ...
    /* Initializations */
    if (verbosity >= 2) sim_set_dumpfile(stdout);
    sim_init();
...
static void usage(char *name) {
    ...
    printf(
        ...
        instr_limit);
    printf(
        "   -v n   Set verbosity level to 0 <= n <= 2 [TTY mode only] (default "
        "%d)\n",
        verbosity);
    printf(
        "   -t     Test result against ISA simulator (yis) [TTY mode only]\n");
    exit(0);
}
...
static char *format_f() {
    ...
    sprintf(status_msg, "%s %s %s %s %s", iname(HPACK(icode, ifun)),
            reg_name(ra), reg_name(rb), valcstring, valpstring);
    return status_msg;
}
...
static void update_state() {
    ...
    cc = cc_in;
    /* Writeback */
    if (destE != REG_NONE) set_reg_val(reg, destE, vale);
    if (destM != REG_NONE) set_reg_val(reg, destM, valm);
...
static byte_t sim_step() {
    ...
    cc_in = cc;
    if (gen_set_cc()) cc_in = compute_cc(alufun, aluA, aluB);
...
word_t sim_run(word_t max_instr, byte_t *statusp, cc_t *ccp) {
    ...
    while (icount < max_instr) {
        run_status = sim_step();
        icount++;
        if (run_status != STAT_AOK) break;
    }
    if (statusp) *statusp = run_status;
    if (ccp) *ccp = cc;
    return icount;
}
...
void report_line(word_t line_no, word_t addr, char *hex, char *text) {
    int code;
    sprintf(tcl_msg, "addCodeLine %lld %lld {%s} {%s}", line_no, addr, hex,
            text);
    code = Tcl_Eval(sim_interp, tcl_msg);
    if (code != TCL_OK) {
